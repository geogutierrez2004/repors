...
export function registerDashboardHandlers(
  ...
  ipcMain.handle(IPC_CHANNELS.SHELVES_CREATE, (_event, payload: unknown) =>
    guard(async () => {
      try {
        ...
        return ok(await dashboardService.createShelf(sessionId, name));
      } catch (e) {
        return handleError(e);
      }
    }));
  ...
  ipcMain.handle(IPC_CHANNELS.SHELVES_RENAME, (_event, payload: unknown) =>
    guard(async () => {
      try {
        ...
        return ok(await dashboardService.renameShelf(sessionId, shelfId, name));
      } catch (e) {
        return handleError(e);
      }
    }));
  ...
  ipcMain.handle(IPC_CHANNELS.STORAGE_GET_PATH, (_event, payload: unknown) =>
    guard(async () => {
      try {
        const { sessionId } = SessionIdOnlySchema.parse(payload);
        return ok(dashboardService.getStoragePath(sessionId));
      } catch (e) {
        return handleError(e);
      }
    }));

  ipcMain.handle(IPC_CHANNELS.STORAGE_CREATE_SHARE, (_event, payload: unknown) =>
    guard(async () => {
      try {
        const { sessionId } = SessionIdOnlySchema.parse(payload);
        return ok(await dashboardService.createShare(sessionId));
      } catch (e) {
        return handleError(e);
      }
    }));

  ipcMain.handle(IPC_CHANNELS.STORAGE_REMOVE_SHARE, (_event, payload: unknown) =>
    guard(async () => {
      try {
        const { sessionId } = SessionIdOnlySchema.parse(payload);
        return ok(await dashboardService.removeShare(sessionId));
      } catch (e) {
        return handleError(e);
      }
    }));

  ipcMain.handle(IPC_CHANNELS.STORAGE_OPEN_FOLDER, (_event, payload: unknown) =>
    guard(async () => {
      try {
        const { sessionId } = SessionIdOnlySchema.parse(payload);
        return ok(await dashboardService.openStorageFolder(sessionId));
      } catch (e) {
        return handleError(e);
      }
    }));
